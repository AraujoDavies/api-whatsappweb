...
def test_if_print_doesnt_works_without_browser():
    with TestClient(app) as ac:
        response = ac.get(
            f'/screenshot?phone_number={getenv("VALID_PHONE_NUMBER")}'
        )
...
def test_stop_route_without_open_browser():
    with TestClient(app) as ac:
        response = ac.get(f'/stop?phone_number={getenv("VALID_PHONE_NUMBER")}')
...
def test_start_route():
    with TestClient(app) as ac:
        response = ac.get(
            f'/start?phone_number={getenv("VALID_PHONE_NUMBER")}'
        )
    assert (
...
def test_browsers_instacited():
    ...
    assert f'["{getenv("VALID_PHONE_NUMBER")}"]' in response.text
...
def test_if_screenshot_works_like_hoped():
    ...
    assert (
        '<h1>  </h1> <img src="screenshot/user_5511930628076.png" alt="file not found.">'
        in response.text
    )
...
def test_if_find_chat_and_if_message_is_sended():
    with TestClient(app) as ac:
        findchat = ac.post(
            '/find-chat',
            json={
                'phone_number': getenv('VALID_PHONE_NUMBER'),
        ...
        stop = ac.get(
            f'/stop?phone_number={getenv("VALID_PHONE_NUMBER")}'
